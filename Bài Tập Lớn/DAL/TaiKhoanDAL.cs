...
namespace Bài_Tập_Lớn.DAL
{
    public class TaiKhoanDAL
    {
        ...
        {
            string sql = @"SELECT ISNULL(MAX(CAST(SUBSTRING(ma_tk, 3, LEN(ma_tk)) AS INT)), 0) FROM tai_khoan";
            try
            {
            ...
            {
                throw new Exception("Lỗi khi sinh mã tài khoản: " + ex.Message);
            }
        }

        public bool TaoAdminMacDinh()
        {
            string kiemTraSql = @"SELECT COUNT(*) FROM tai_khoan WHERE vai_tro = N'Admin'";
            try
            {
                using (IDbConnection conn = DBConnection.Instance.GetConnection())
                {
                    ...
                    if (count == 0)
                    {
                        string insertSql = @"INSERT INTO tai_khoan(ma_tk, ten_dang_nhap, mat_khau, vai_tro, ma_nv)
                            VALUES ('TK01', 'admin', 'admin123', N'Admin', NULL)";
            ...
            {
                throw new Exception("Lỗi khi tạo admin mặc định: " + ex.Message);
            }
        }
        ...
        {
            string sql = @"SELECT * FROM tai_khoan ORDER BY ma_tk";
            try
            {
                using (IDbConnection conn = DBConnection.Instance.GetConnection())
                {
        ...
        public TaiKhoanDTO LayTheoMaTK(string maTK)
        {
            ...
            {
                throw new Exception("Lỗi khi lấy tài khoản theo mã: " + ex.Message);
            }
        }
        ...
        public TaiKhoanDTO LayTheoTenDangNhap(string tenDangNhap)
        {
            ...
            {
                throw new Exception("Lỗi khi lấy tài khoản theo tên đăng nhập: " + ex.Message);
            }
        }
        ...
        {
            string sql = @"INSERT INTO tai_khoan(ma_tk, ten_dang_nhap, mat_khau, vai_tro, ma_nv)
                VALUES(@MaTK, @TenDangNhap, @MatKhau, @VaiTro, @MaNV)";
            try
            {
                using (IDbConnection conn = DBConnection.Instance.GetConnection())
                {
        ...
        public bool XoaTaiKhoan(string maTK)
        {
            ...
            try
            {
                using (IDbConnection conn = DBConnection.Instance.GetConnection())
                {
                    int rows = conn.Execute(sql, new { MaTK = maTK });
                    return rows > 0;
                }
            }
            catch (Exception ex)
        ...
        public List<TaiKhoanDTO> TimKiem(string keyword)
        {
            string sql = @"SELECT * FROM tai_khoan                 
                        ...
                        OR vai_tro COLLATE Vietnamese_CI_AI LIKE @Keyword COLLATE Vietnamese_CI_AI";
            try
            {
                using (IDbConnection conn = DBConnection.Instance.GetConnection())
                {
